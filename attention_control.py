...
tokenizer = StableDiffusionPipeline.from_pretrained("stabilityai/stable-diffusion-2-base").tokenizer
...
class LocalBlend:
    ...
    def __init__(self, prompts: List[str], words: [List[List[str]]],tokenizer=None,max_num_words=77,num_ddim_steps=50, substruct_words=None, start_blend=0.99, th=(.3, .3)):
        alpha_layers = torch.zeros(len(prompts),  1, 1, 1, 1,max_num_words)
        for i, (prompt, words_) in enumerate(zip(prompts, words)):
            if type(words_) is str:
                words_ = [words_]
...
def run_and_display(prompts, controller, num_diffusion_steps = 50,guidance_scale=7.5, ldm_stable=None,latent=None, run_baseline=False, generator=None,uncond_embeddings=None ):
    if run_baseline:
        print("w.o. prompt-to-prompt")
        images, latent = run_and_display(prompts, EmptyControl(), latent=latent, run_baseline=False, generator=generator)
        print("with prompt-to-prompt")
    images, x_t = ptp_utils.text2image_ldm_stable(ldm_stable, prompts, controller, latent=latent, num_inference_steps=num_diffusion_steps, guidance_scale=guidance_scale, generator=generator, uncond_embeddings=uncond_embeddings)
    # ptp_utils.view_images(images)
    return images, x_t

def make_controller(prompts: List[str], is_replace_controller: bool, cross_replace_steps: Dict[str, float], self_replace_steps: float, blend_words=None, equilizer_params=None,num_ddim_steps=50,tokenizer=None) -> AttentionControlEdit:
    if blend_words is None:
        lb = None
    else:
        lb = LocalBlend(prompts, blend_words,tokenizer=tokenizer,num_ddim_steps=num_ddim_steps)
