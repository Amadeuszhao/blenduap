...
def latent_attack(
    ...
    controller = make_controller(prompts, True, cross_replace_steps, self_replace_steps, blend_word,num_ddim_steps=num_inference_steps,tokenizer=tokenizer,)
    images, _ = run_and_display(prompts, controller,num_diffusion_steps = num_inference_steps, run_baseline=False, latent=x_t, uncond_embeddings=uncond_embeddings,ldm_stable=model,perturb = None)
    new_prompts = [ori_label_name,ori_label_name,]
    perturb = torch.zeros(1, 4, 64, 64).cuda()
    perturb.requires_grad_(True)
    optimizer= optim.AdamW([perturb], lr=5e-2)
    for _ in tqdm(range(iterations)):
        controller = make_controller(prompts, True, cross_replace_steps, self_replace_steps, blend_word,num_ddim_steps=num_inference_steps,tokenizer=tokenizer)
        images, _ = run_and_display(new_prompts, controller,num_diffusion_steps = num_inference_steps, run_baseline=False, latent=x_t, uncond_embeddings=uncond_embeddings,ldm_stable=model,perturb = perturb)
